...
namespace Ligric.Service.CryptoApisService.Api.Services
{
	...
	public class BinanceFuturesOrdersService : BinanceFuturesOrders.BinanceFuturesOrdersBase
	{
		...
		[Authorize]
		public override async Task OrdersSubscribe(Empty request, IServerStreamWriter<OrdersChanged> responseStream, ServerCallContext context)
		{
		...
		{
			_ordersSubscriptions.SetSubscribedStream(request.UserId, request.UserApiId, out var streamSubscriptionId, out var chainSessionId);
