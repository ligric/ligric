...
namespace Ligric.Service.CryptoApisService.Api.Services
{
	...
	public class BinanceFuturesPositionsService : BinanceFuturesPositions.BinanceFuturesPositionsBase
	{
		...
		[Authorize]
		public override async Task PositionsSubscribe(Empty request, IServerStreamWriter<PositionsChanged> responseStream, ServerCallContext context)
		{
		...
		{
			_positionSubscriptions.SetSubscribedStream(request.UserId, request.UserApiId, out var streamSubscriptionId, out var chainSessionId);
