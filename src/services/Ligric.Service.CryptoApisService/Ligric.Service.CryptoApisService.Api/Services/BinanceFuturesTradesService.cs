...
namespace Ligric.Service.CryptoApisService.Api.Services
{
	...
	public class BinanceFuturesTradesService : BinanceFuturesTrades.BinanceFuturesTradesBase
	{
		...
		[Authorize]
		public override async Task TradesSubscribe(Empty request, IServerStreamWriter<TradesChanged> responseStream, ServerCallContext context)
		{
		...
		{
			_tradeSubscriptions.SetSubscribedStream(request.UserId, request.UserApiId, out var streamSubscriptionId, out var chainSessionId);
