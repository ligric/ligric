...
namespace Ligric.Service.CryptoApisService.Application
{
	public interface ITemporaryUserFuturesObserver
	{
		...
		IObservable<(IEnumerable<long> UserIds, Guid ExchangeId, NotifyDictionaryChangedEventArgs<string, byte> EventArgs)> GetLeveragesAsObservable(long userId, long userApiId);
	}
}
